...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.cls_classifier_merge = MLP(15, 5, [15 * args.smallmid_ratio])
    ...
    def forward(self, frames):
        ...
        input1[:, 4:5, :] = output1[:, 2:3, :]
        input2[:, 0:1, :] = output2[:, 0:1, :]
        input2[:, 2:3, :] = output2[:, 1:2, :]
        input2[:, 4:5, :] = output2[:, 2:3, :]
        input3[:, 0:1, :] = output3[:, 0:1, :]
        input3[:, 3:5, :] = output3[:, 1:3, :]

        input_merge = torch.cat([input1, input2, input3], dim=1)
        ...
        return cls_output1, cls_output2, cls_output3, cls_output
