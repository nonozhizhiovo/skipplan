...
logname = 'exptower4_' + time_pre + '_' + str(args.dataset_mode) + '_' + str(args.max_traj_len)
...
def train(args, train_loader, model, optimizer, epoch, criterion, tb_logger):
    ...
    for i, (_, _, frames, _, lowlevel_labels, _) in enumerate(train_loader):
        ...
        output2, output3, output = model(frames)
        ...
        lowlevel_labels2 = torch.cat([lowlevel_labels[:, 0:2], lowlevel_labels[:, 3:4]], dim=1)
        ...
        lowlevel_labels3 = torch.cat([lowlevel_labels[:, 0:1], lowlevel_labels[:, 2:4]], dim=1)
